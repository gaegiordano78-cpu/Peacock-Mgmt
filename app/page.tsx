...
const PAG_COLOR = { pagato: "#16A34A", "da pagare": "#DC2626", "in attesa": "#D97706" };
const PAG_BG    = { pagato: "#F0FDF4", "da pagare": "#FEF2F2", "in attesa": "#FFFBEB" };
...
const Chip = ({ children, active, onClick }) => (
  <button onClick={onClick} style={{ flexShrink: 0, padding: "6px 14px", borderRadius: 100, fontSize: 11, fontWeight: active ? 600 : 400, cursor: "pointer", border: active ? "1.5px solid #1C1714" : "1.5px solid #E5E0D8", background: active ? "#1C1714" : "transparent", color: active ? "#FFF" : "#6B6560", fontFamily: "inherit", transition: "all 0.15s" }}>{children}</button>
);
...
const CalcRow = ({ label, val, bold, big }) => (
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0" }}>
    ...
    <span style={{ fontSize: big ? 20 : bold ? 13 : 12, fontWeight: big ? 800 : bold ? 600 : 400, color: "#1C1714", letterSpacing: big ? "-0.03em" : "0" }}>{val}</span>
  </div>
);
...
const Section = ({ title, children, action }) => (
  <div style={{ marginBottom: 20 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", color: "#9C948A", textTransform: "uppercase" }}>{title}</div>
      {action}
    </div>
    <div style={{ background: "#FFFFFF", borderRadius: 16, border: "1px solid #F0EAE0", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
      {children}
    </div>
  </div>
);
...
const SelectField = ({ label, value, onChange, options }) => (
  <div style={{ marginBottom: 14 }}>
    ...
    <select value={value} onChange={e => onChange(e.target.value)}
      style={{ width: "100%", background: "#FAFAF8", border: "1.5px solid #EAE4DC", borderRadius: 12, color: "#1C1714", fontSize: 14, padding: "10px 14px", fontFamily: "inherit", boxSizing: "border-box", outline: "none", appearance: "none" }}>
...
const CalBtn = ({ icon, label, sub, onClick, loading }) => (
  <button onClick={onClick} disabled={loading}
    style={{ width: "100%", marginBottom: 8, padding: "12px 14px", background: loading ? "#F5F0EA" : "#FAFAF8", border: "1.5px solid #EAE4DC", borderRadius: 14, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 12, fontFamily: "inherit" }}>
    <span style={{ fontSize: 20 }}>{loading ? "⏳" : icon}</span>
    <div style={{ textAlign: "left", flex: 1 }}>
      <div style={{ fontSize: 13, fontWeight: 500, color: "#1C1714" }}>{label}</div>
      <div style={{ fontSize: 10, color: "#9C948A", marginTop: 1 }}>{sub}</div>
    </div>
...
const PrimaryBtn = ({ children, onClick, color = "#1C1714", disabled }) => (
  <button onClick={onClick} disabled={disabled}
    style={{ width: "100%", marginTop: 8, padding: "15px", background: disabled ? "#E5E0D8" : color, border: "none", borderRadius: 16, color: "#FFF", fontSize: 14, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer", fontFamily: "inherit", letterSpacing: "0.01em" }}>
    {children}
  </button>
);
...
const GhostBtn = ({ children, onClick }) => (
  <button onClick={onClick}
    style={{ width: "100%", marginTop: 8, padding: "12px", background: "transparent", border: "1.5px solid #EAE4DC", borderRadius: 14, color: "#6B6560", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
    {children}
  </button>
);
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", background: "#F7F3EE", minHeight: "100vh", maxWidth: 430, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: toastErr ? "#DC2626" : "#1C1714", color: "#FFF", padding: "10px 20px", borderRadius: 100, fontSize: 12, fontWeight: 500, zIndex: 100, boxShadow: "0 4px 24px rgba(0,0,0,0.15)", whiteSpace: "nowrap" }}>
          {toast}
        </div>
      )}

      {/* HEADER */}
      <div style={{ background: "#FFFFFF", borderBottom: "1px solid #F0EAE0", position: "sticky", top: 0, zIndex: 20, boxShadow: "0 1px 8px rgba(0,0,0,0.04)" }}>
        <div style={{ padding: "20px 20px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {view !== "lista" && view !== "modelle" && view !== "calcolatrice" && (
              <button onClick={backView} style={{ padding: "8px 16px", borderRadius: 100, border: "1.5px solid #E8E2DA", background: "transparent", color: "#6B6560", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>← Indietro</button>
            )}
            {view === "lista" && (
              <>
                {userRuolo === "admin" && (
        ...
        {pageTitle() && (
          <div style={{ padding: "0 20px 14px" }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: "#1C1714", letterSpacing: "-0.02em" }}>{pageTitle()}</div>
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 30 }}>
        ...
        {view === "lista" && (
          <div>
            {/* Stats */}
            <div style={{ background: "#FFFFFF", borderBottom: "1px solid #F0EAE0", display: "flex" }}>
              {[
                ...
                <div key={i} style={{ flex: 1, padding: "16px 8px", textAlign: "center", borderRight: i < 2 ? "1px solid #F0EAE0" : "none" }}>
                  ...
                  <div style={{ fontSize: 9, color: "#9C948A", marginTop: 3, textTransform: "uppercase", letterSpacing: "0.08em" }}>{s.label}</div>
                </div>
              ))}
            </div>

            {/* Filtri */}
            <div style={{ padding: "12px 16px", display: "flex", gap: 6, overflowX: "auto", background: "#FFFFFF", borderBottom: "1px solid #F0EAE0" }}>
            ...
            <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: 10 }}>
              {jobFiltrati.length === 0 && (
                <div style={{ textAlign: "center", color: "#9C948A", padding: "40px 0", fontSize: 14 }}>Nessun job trovato</div>
              )}
              {jobFiltrati.map(job => (
                <div key={job.id} onClick={() => { setSelectedJob(job); setView("dettaglio"); }}
                  ...
                  <div style={{ textAlign: "right", marginLeft: 12, flexShrink: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 700, color: "#1C1714", letterSpacing: "-0.02em" }}>{fmt(calcNetto(job))}</div>
                    <div style={{ fontSize: 10, color: "#9C948A", marginTop: 2 }}>netto</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          return (
            <div style={{ padding: "20px 16px" }}>
              ...
              <PaddedSection title="Pagamento">
                ...
                <div style={{ height: 8 }} />
                <div style={{ background: "#F7F3EE", borderRadius: 12, padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#1C1714" }}>Netto da pagare</span>
                  <span style={{ fontSize: 22, fontWeight: 800, color: "#1C1714", letterSpacing: "-0.03em" }}>{fmt(calcNetto(job))}</span>
                </div>
                {job.stato_pagamento !== "pagato" ? (
                  <button onClick={() => { marcaPagato(job.id); setSelectedJob({ ...job, stato_pagamento: "pagato" }); }}
                    style={{ width: "100%", marginTop: 10, padding: "13px", background: "#16A34A", border: "none", borderRadius: 14, color: "#FFF", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                    ✓ Marca come pagato
                  </button>
                ) : (
              ...
              <Section title="💬 WhatsApp">
                <div style={{ padding: "14px 16px", background: "#FAFAF8", fontSize: 13, color: "#3C3530", lineHeight: 1.6, whiteSpace: "pre-wrap", borderRadius: 16 }}>
...
A domani 🤍`}
        ...
        {view === "contratto" && selectedJob && (() => {
          ...
          return (
            <div style={{ padding: "16px" }}>
              <div style={{ background: "#FFFFFF", borderRadius: 16, border: "1px solid #F0EAE0", padding: "20px 18px", fontSize: 12, color: "#1C1714", lineHeight: 1.8, whiteSpace: "pre-wrap", fontFamily: "Georgia, serif", boxShadow: "0 1px 4px rgba(0,0,0,0.04)", marginBottom: 16 }}>
                {testo}
              </div>
              <button onClick={() => copiaContratto(job)}
                style={{ width: "100%", padding: "15px", background: contrattoCopied ? "#16A34A" : "#1C1714", border: "none", borderRadius: 16, color: "#FFF", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
              ...
              <Section title="Polas"
                action={mod.link_polas ? (
                  <a href={mod.link_polas} target="_blank" rel="noreferrer"
                    style={{ fontSize: 11, color: "#C4A882", fontWeight: 600, textDecoration: "none" }}>
                    Apri Drive →
                  </a>
                ) : null}>
                <div style={{ padding: "14px 16px" }}>
                  {mod.link_polas ? (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <a href={mod.link_polas} target="_blank" rel="noreferrer"
                        style={{ width: 40, height: 40, background: "#F7F3EE", border: "1.5px solid #EAE4DC", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, textDecoration: "none" }}>
                        📁
                      </a>
                    </div>
                  ) : (
                      ...
                      {mod.contratto_firma ? (
                        <>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                            <span style={{ display: "inline-flex", alignItems: "center", padding: "4px 12px", borderRadius: 100, fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: cc.color, background: cc.bg, border: `1px solid ${cc.color}33` }}>
                              {tipo}
                            </span>
                            {alert && (
                              <span style={{ fontSize: 11, fontWeight: 600, color: alert === "scaduto" ? "#DC2626" : "#D97706", background: alert === "scaduto" ? "#FEF2F2" : "#FFFBEB", padding: "3px 10px", borderRadius: 100 }}>
                  ...
                  {mj.map((job, i) => (
                    <div key={job.id}>
                      <div onClick={() => { setSelectedJob(job); setView("dettaglio"); }}
                        ...
                        <div>
                          <div style={{ fontSize: 13, fontWeight: 500, color: "#1C1714", marginBottom: 2 }}>{job.titolo}</div>
                          <div style={{ fontSize: 11, color: "#9C948A" }}>{job.cliente} · {fmtDate(job.data_shooting)}</div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: "#1C1714" }}>{fmt(calcNetto(job))}</div>
                          <Badge label={job.stato_pagamento} color={PAG_COLOR[job.stato_pagamento]} bg={PAG_BG[job.stato_pagamento]} />
                        </div>
                      </div>
        ...
        {view === "nuova_modella" && (
          <div style={{ padding: "16px" }}>
            ...
            <div style={{ height: 8 }} />
            <div style={{ fontSize: 11, fontWeight: 700, color: "#9C948A", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 14 }}>Contratto Agenzia</div>
            ...
            {formJob.fatturato > 0 && (
              <div style={{ background: "#F7F3EE", borderRadius: 16, padding: "14px 16px", marginBottom: 14, border: "1.5px solid #EAE4DC" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#1C1714" }}>Netto modella</span>
                  <span style={{ fontSize: 16, fontWeight: 800, color: "#1C1714" }}>{fmt(calcNetto(formJob))}</span>
                </div>
              </div>
            )}
